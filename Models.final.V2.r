...
for (i in 1:length(folds)) {
  ...
  cph <- coxph(Surv(Time.elapsed, status == 1) ~ ., data = train_data, model = TRUE, x = TRUE, y = TRUE)
  test_predictions <- predict(cph, newdata = test_data, type = "lp")
  c_index <- concordance(Surv(test_data$Time.elapsed, test_data$status == 1) ~ test_predictions)$concordance
  c_index_list <- c(c_index_list, c_index)
  cat("Fold", i, "- C-index:", round(c_index, 3), "\n")
}

# Fold 1 - C-index: 0.368 
# Fold 2 - C-index: 0.56 
# Fold 3 - C-index: 0.423 
# Fold 4 - C-index: 0.351 
# Fold 5 - C-index: 0.289 
# Fold 6 - C-index: 0.267 
# Fold 7 - C-index: 0.323 
# Fold 8 - C-index: 0.396 
# Fold 9 - C-index: 0.416
# Fold 10 - C-index: 0.401 

# Average C-index across folds
mean_c_index <- mean(c_index_list)
cat("Average C-index from cross-validation:", mean_c_index, "\n")
#Average C-index from cross-validation: 0.3894012  

# Standard deviation of C-index across folds
sd_c_index <- sd(c_index_list)
cat("Standard Deviation of C-index from cross-validation:", sd_c_index, "\n")
#Standard Deviation of C-index from cross-validation:0.07312279
...
###Sensitivity analysis with clustering by Individuals (first quit attempt only)######
df<- read.csv("unimputed_dataset_final_userid.csv")
str(df)
df$X<- NULL

###reinserting factor variables as factor
df[,c(2:6, 8:12, 14:16, 20:30, 32, 33,36:38, 40)] <- lapply(df[,c(2:6, 8:12, 14:16, 20:30, 32, 33,36:38, 40)], as.factor)
...
#scaling numeric variables
df[,c(7,13,17,18,19,31,34, 35)] <- lapply(df[,c(7,13,17,18,19,31,34, 35)], function(x) c(scale(x)))
str(df)

#Limiting to first quit attempt
df <- df %>%
  group_by(userid) %>%
  slice_head(n = 1) %>%
  ungroup()
...
#saving dataset
write.csv(df, "imputed_dataset_firstquit.csv")
...
# Create folds for outer cross-validation
set.seed(123)
folds <- createFolds(df$status, k = outerfold, list = TRUE)

nested_cv_gbm_limited <- list(c_index = numeric(), interaction.depth = numeric(), n.trees = numeric(), 
                              shrinkage = numeric(), n.minobsinnode = numeric(), train_data = list(),test_data = list())
...
for (i in 1:outerfold) {
  ...
  best_iter <- gbm.perf(best_gbm_model_limited, method = "test", plot.it = FALSE)
  test_pred <- predict(best_gbm_model_limited, newdata = test_data, n.trees = best_iter)
  test_c_index <- rcorr.cens(-test_pred, Surv(test_data$Time.elapsed, test_data$status == 1))["C Index"]
  
  nested_cv_gbm_limited$c_index <- c(nested_cv_gbm_limited$c_index, test_c_index)
  nested_cv_gbm_limited$interaction.depth <- c(nested_cv_gbm_limited$interaction.depth, best_params$interaction.depth)
  nested_cv_gbm_limited$n.trees <- c(nested_cv_gbm_limited$n.trees, best_params$n.trees)
  nested_cv_gbm_limited$shrinkage <- c(nested_cv_gbm_limited$shrinkage, best_params$shrinkage)
  nested_cv_gbm_limited$n.minobsinnode <- c(nested_cv_gbm_limited$n.minobsinnode, best_params$n.minobsinnode)
  nested_cv_gbm_limited$train_data[[i]] <- train_data
  nested_cv_gbm_limited$test_data[[i]] <- test_data
}

###results
# best parameters and C-index for each outer fold
for (i in 1:outerfold) {
  cat("  Fold:", i, "C-index:", nested_cv_gbm_limited$c_index[i], "\n\n", 
      "interaction depth:", nested_cv_gbm_limited$interaction.depth[i], "\n\n",
      "ntrees:", nested_cv_gbm_limited$n.trees[i], "\n\n",
      "learning rate", nested_cv_gbm_limited$shrinkage[i], "\n\n",
      "n.minobsinnode", nested_cv_gbm_limited$n.minobsinnode[i], "\n\n"
  )
}
#Fold: 1 C-index: 0.6069182 interaction depth: 1 ntrees: 100 learning rate 0.1  n.minobsinnode 10 
#Fold: 2 C-index: 0.6184211 interaction depth: 5 ntrees: 500 learning rate 0.01 n.minobsinnode 10 
#Fold: 3 C-index: 0.6617211 interaction depth: 5 ntrees: 200 learning rate 0.1   n.minobsinnode 10 
#Fold: 4 C-index: 0.6619718 interaction depth: 5 ntrees: 200 learning rate 0.1   n.minobsinnode 10 
#Fold: 5 C-index: 0.5868263 interaction depth: 1 ntrees: 200 learning rate 0.01 n.minobsinnode 10 
#Fold: 6 C-index: 0.5145119 interaction depth: 3 ntrees: 100 learning rate 0.01 n.minobsinnode 10 
#Fold: 7 C-index: 0.548105   interaction depth: 5 ntrees: 200 learning rate 0.01 n.minobsinnode 10 
#Fold: 8 C-index: 0.6130952 interaction depth: 3 ntrees: 100 learning rate 0.01 n.minobsinnode 10 
#Fold: 9 C-index: 0.5952381 interaction depth: 1 ntrees: 200 learning rate 0.01 n.minobsinnode 10 
#Fold: 10 C-index: 0.566343 interaction depth: 3 ntrees: 500 learning rate 0.1   n.minobsinnode 10 

# Summary of the C-index
cat("Mean C-index across all folds:", mean(nested_cv_gbm_limited$c_index), "\n")
#Mean C-index across all folds: 0.5973152   
cat("Standard Deviation of C-index across all folds:", sd(nested_cv_gbm_limited$c_index), "\n")
#Standard Deviation of C-index across all folds:0.04650797 

##SHAP analysis
best_fold_gbm_limited <- which.max(nested_cv_gbm_limited$c_index)
best_index_gbm_limited <- max(nested_cv_gbm_limited$c_index)
best_interaction.depth_gbm_limited <- nested_cv_gbm_limited$interaction.depth[best_fold_gbm_limited]
best_ntree_gbm_limited <- nested_cv_gbm_limited$n.trees[best_fold_gbm_limited]
best_shrinkage_gbm_limited <- nested_cv_gbm_limited$shrinkage[best_fold_gbm_limited]
best_n.minobsinnode_gbm_limited <- nested_cv_gbm_limited$n.minobsinnode[best_fold_gbm_limited]

best_train_data_gbm_limited <- nested_cv_gbm_limited$train_data[[best_fold_gbm_limited]]
best_test_data_gbm_limited <- nested_cv_gbm_limited$test_data[[best_fold_gbm_limited]]
X_test_best_gbm_limited <- best_test_data_gbm_limited[, setdiff(names(best_test_data_gbm_limited), c("Time.elapsed", "status"))]
y_test_best_gbm_limited <- best_test_data_gbm_limited[, c("Time.elapsed", "status")]

# Fit the best model using the training data from the best fold
set.seed(123)
best_gbm_limited <- gbm(
  formula = Surv(Time.elapsed, status == 1) ~ .,
  data = best_train_data_gbm_limited,
  distribution = "coxph",
  n.trees = best_ntree_gbm_limited,
  interaction.depth = best_interaction.depth_gbm_limited,
  shrinkage = best_shrinkage_gbm_limited,
  n.minobsinnode = best_n.minobsinnode_gbm_limited
)

#####SHAP analysis
# Define the prediction function
predict_function <- function(model, newdata) {
  predict(model, newdata = newdata, n.trees = best_ntree_gbm_limited)
}

# Compute SHAP values
set.seed(123)
shap_values_gbm_limited <- fastshap::explain(
  object = best_gbm_limited,
  X = X_test_best_gbm_limited,
  pred_wrapper = predict_function,
  nsim = 100
)
print(shap_values_gbm_limited)

shp.gbm_limited <- shapviz(shap_values_gbm_limited, X= X_test_best_gbm_limited)
str(shp.gbm_limited$X)
...
colnames(shp.gbm_limited$X)[colnames(shp.gbm_limited$X) %in% names(rename_map)] <- rename_map[colnames(shp.gbm_limited$X) %in% names(rename_map)]
str(shp.gbm_limited$X)
colnames(shp.gbm_limited[["S"]])[colnames(shp.gbm_limited[["S"]]) %in% names(rename_map)] <- rename_map[colnames(shp.gbm_limited[["S"]]) %in% names(rename_map)]
colnames(shp.gbm_limited[["S"]])

#feature importance
shp.gbm.imp_limited<- sv_importance(shp.gbm_limited)
gbm.imp_limited<- data.frame(shp.gbm.imp_limited$data)
gbm.imp_limited$value <- round(gbm.imp_limited$value,2)
G1_gbm_limited<- ggplot(data=gbm.imp_limited,aes(x=reorder(feature,value),y=value))+
  geom_bar(fill= "navyblue", stat="identity")+
  geom_text(aes(label= value), y=0.01,color="White",size=3)+
  coord_flip()+
  labs(y="mean SHAP value",
  ...
  theme(text = element_text(size= 12), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))
print(G1_gbm_limited)

shp.gbm_limited.summary<- sv_importance(shp.gbm_limited, kind = "beeswarm", show_numbers = TRUE)
print(shp.gbm_limited.summary)
...
p4<- sv_dependence(shp.gbm, v = "Pod depletion time", color_var = NULL)
...
grid1<- gridExtra::grid.arrange(p1,p2,p3, p4, p7, p13, ncol=2)
grid2<- gridExtra::grid.arrange(p5,p14,p8,p10, ncol=2)
grid3<- gridExtra::grid.arrange(p6,p12, p9, p11, ncol=2)
...
tiff("combined_grids.tiff", width = 12, height = 14, units = "in", res = 300, compression = "lzw")
...
sv_waterfall(shp.gbm, row_id = 27)


###Interactions
predictor <- Predictor$new(
  model = best_gbm, 
  data = X_test_best_gbm,
  y = y_test_best_gbm$Time.elapsed, type = "response")
interaction <- Interaction$new(predictor)
plot(interaction)

#Self-confidence in quitting
interact <- Interaction$new(predictor, feature = "ecusv_3")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
...
#Intention to quit
interact <- Interaction$new(predictor, feature = "ecu17_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
interact <- Interaction$new(predictor, feature = "epp13_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
interact <- Interaction$new(predictor, feature = "epp14_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Pod depletion time
interact <- Interaction$new(predictor, feature = "epp15_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
# Time to first vape
interact <- Interaction$new(predictor, feature = "ecu11b_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Past 30-day alcohol drinking
interact <- Interaction$new(predictor, feature = "cur_alc_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Past-month vaping frequency
interact <- Interaction$new(predictor, feature = "ecu8_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Initial craving during challenge
interact <- Interaction$new(predictor, feature = "first_craving")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Initial mood during challenge
interact <- Interaction$new(predictor, feature = "first_mood")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#craving trend during challenge
interact <- Interaction$new(predictor, feature = "craving_trend")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Mood trend during challenge
interact <- Interaction$new(predictor, feature = "mood_trend")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Reasons for quitting-others
interact <- Interaction$new(predictor, feature = "ecusv_1___13")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
interact <- Interaction$new(predictor, feature = "ecu16_1")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#Sexual orientation
interact <- Interaction$new(predictor, feature = "sexorient")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#gender
interact <- Interaction$new(predictor, feature = "gender")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#race
interact <- Interaction$new(predictor, feature = "race")
feature_pairs <- interact[["results"]][[".feature"]]
interaction_values <- interact[["results"]][[".interaction"]]
interaction_df <- data.frame(
  feature_pair = feature_pairs,
...
#kaplan-Meier for initial craving
...
summary(df$race)
df <- df %>%
  mutate(race = factor(race, levels = c(0, 1), labels = c("Non-White", "White")))
fit4 <- survfit(Surv(Time.elapsed, status == 1) ~ race, data = df)
km_plot4<- ggsurvplot(fit4, data = df,
                      pval = TRUE,
                      conf.int = TRUE, 
                      risk.table = TRUE,
                      xlab = "Time (days)",
