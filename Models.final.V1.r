...
label_grid <- function(grob, label) {
  arrangeGrob(
    grob,
    top = textGrob(label,
                   x = unit(0.01, "npc"),  
...
grid1_labeled <- label_grid(grid1, "A)")
grid2_labeled <- label_grid(grid2, "B)")
grid3_labeled <- label_grid(grid3, "C)")

pdf("multiple_images.pdf", width = 12, height = 10)
grid.draw(grid1_labeled)
grid.newpage()
grid.draw(grid2_labeled)
grid.newpage()
grid.draw(grid3_labeled)
dev.off()
