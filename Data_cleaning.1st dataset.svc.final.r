...
time_points <- c(1, 7, 14, 30, 60)

duration_comparison <- lapply(time_points, function(t) {
  df_tmp <- df1 %>%
    filter(!is.na(Time.elapsed)) %>%
    mutate(lasted = ifelse(Time.elapsed >= t, 1, 0))
    summary_tbl <- df_tmp %>%
    group_by(status) %>%
    summarise(
      total = n(),
      lasted_count = sum(lasted),
      percent_lasted = 100 * lasted_count / total,
      .groups = "drop"
    )
    tab <- table(df_tmp$status, df_tmp$lasted)
    pval <- if (any(tab < 5)) {
